...
public class CardDimensionManager {
    ...
    // the return portal lives in the overworld, always active, no ritual needed
    private final Set<Long> returnFrameBlocks = new HashSet<>();
    private final Set<Long> returnInteriorBlocks = new HashSet<>();
    ...
    public void setup() {
        ...
        // Build the return portal (in the overworld) if it doesn't exist yet - covers
        // both a genuinely fresh setup and upgrading from a version that didn't have
        // a return portal at all.
        if (returnFrameBlocks.isEmpty()) {
            buildReturnPortal();
            save();
        }
    }
    ...
    private void buildPortalStructure(int surfaceY) {
        ...
        int baseX = 0;
        int baseZ = 0;
        int groundY = findGroundY(world, baseX, baseZ, surfaceY);
        int baseY = groundY + 1;
    ...
    /**
     * Builds the always-active return portal in the overworld, a short distance from
     * its vanilla spawn point (not directly on top of it, so it doesn't block/overwrite
     * the exact spawn tile). No SpecialMatter ritual needed for this one - it's lit
     * immediately.
     */
    private void buildReturnPortal() {
        World overworld = Bukkit.getWorld(getOverworldName());
        ...
        int baseX = spawn.getBlockX() + 6; // offset so it doesn't sit on top of the exact spawn tile
        int baseZ = spawn.getBlockZ();
        int groundY = findGroundY(overworld, baseX, baseZ, spawn.getBlockY());
        int baseY = groundY + 1;

        returnFrameBlocks.clear();
        returnInteriorBlocks.clear();
        ...
        for (int dx = -halfSpan; dx <= halfSpan; dx++) {
            for (int dy = 0; dy < height + 2; dy++) {
                ...
                int x = baseX + dx;
                int y = baseY + dy;
                Block block = overworld.getBlockAt(x, y, baseZ);
                if (isEdge) {
                    block.setType(Material.OBSIDIAN, false);
                    returnFrameBlocks.add(pack(x, y, baseZ));
                } else {
                    block.setType(Material.NETHER_PORTAL, false); // pre-lit, no ritual needed
                    returnInteriorBlocks.add(pack(x, y, baseZ));
                }
            }
        }
    }

    private int findGroundY(World inWorld, int x, int z, int fallback) {
        Block highest = inWorld.getHighestBlockAt(x, z);
    ...
    public void activate() {
        ...
        for (long packed : interiorBlocks) {
            Location loc = unpack(packed, world);
    ...
    public boolean isFrameBlock(Location loc) {
        if (world != null && loc.getWorld().equals(world) && frameBlocks.contains(pack(loc))) return true;
        World overworld = Bukkit.getWorld(getOverworldName());
        return overworld != null && loc.getWorld().equals(overworld) && returnFrameBlocks.contains(pack(loc));
    }
    ...
    /** True if this location is part of the always-active return portal in the overworld. */
    public boolean isReturnPortalBlock(Location loc) {
        World overworld = Bukkit.getWorld(getOverworldName());
        return overworld != null && loc.getWorld().equals(overworld) && returnInteriorBlocks.contains(pack(loc));
    }
    ...
    private Location unpack(long packed, World inWorld) {
        ...
        if (y >= (1L << 11)) y -= (1L << 12);
        if (z >= (1L << 25)) z -= (1L << 26);
        return new Location(inWorld, x, y, z);
    }
    ...
    private void save() {
        ...
        yaml.set("frame-blocks", toStringList(frameBlocks));
        yaml.set("interior-blocks", toStringList(interiorBlocks));
        yaml.set("return-frame-blocks", toStringList(returnFrameBlocks));
        yaml.set("return-interior-blocks", toStringList(returnInteriorBlocks));
    ...
    private List<String> toStringList(Set<Long> packed) {
        List<String> list = new ArrayList<>();
        for (long p : packed) list.add(Long.toString(p));
        return list;
    }
    ...
    private void load() {
        ...
        frameBlocks.clear();
        for (String s : yaml.getStringList("frame-blocks")) frameBlocks.add(Long.parseLong(s));
        interiorBlocks.clear();
        for (String s : yaml.getStringList("interior-blocks")) interiorBlocks.add(Long.parseLong(s));
        returnFrameBlocks.clear();
        for (String s : yaml.getStringList("return-frame-blocks")) returnFrameBlocks.add(Long.parseLong(s));
        returnInteriorBlocks.clear();
        for (String s : yaml.getStringList("return-interior-blocks")) returnInteriorBlocks.add(Long.parseLong(s));
    }
}
